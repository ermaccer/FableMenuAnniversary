...
{
	static LPDIRECT3DDEVICE9 orig_pDevice = 0;
	static D3DPRESENT_PARAMETERS* orig_pPresentationParameters;

	_asm {
		lea     edx, [esp + 100]
		mov orig_pDevice, eax
		mov orig_pPresentationParameters, edx
		pushad
	}
	ImGui_ImplDX9_InvalidateDeviceObjects();
...
void __stdcall eDirectX9Hook::EndScene(LPDIRECT3DDEVICE9 pDevice)
{
	...
	if (draw)
	{
		...
		Notifications->PushNotification("FableMenuAnniversary %s is running! Press %s to open the menu.", FABLEMENU_VERSION, eKeyboardMan::KeyToString(SettingsMgr->iMenuOpenKey));
		draw = false;
	}
	...
	float len = ImGui::CalcTextSize("FableMenuAnniversary Debug Build").x;
	drawList->AddText({ (x / 2.0f) - (len / 2.0f) ,0 }, IM_COL32(255, 255, 255, 200), "FableMenuAnniversary Debug Build");
#endif // _DEBUG
