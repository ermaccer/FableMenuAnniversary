...
void FableMenu::Process()
{
    ...
    if (m_bNoBodyGuardsLimit)
        Patch<char>(_addr(0x198E176) + 2, 0xFF);
    else
        Patch<char>(_addr(0x198E176) + 2, 2);
...
void FableMenu::SetMessage(eFableMenu_MessageType msg)
{
...
void FableMenu::DrawQuestTab()
{
    ...
    if (ImGui::CollapsingHeader("Status Control"))
    {
        ...
        {       
            // anniversary requires quest calls done from world update for some reason, so using simple messages
            // instead
            if (ImGui::Button("Activate", { -FLT_MIN, 0 }))
                SetMessage(MT_QuestActivate);

            if (ImGui::Button("Deactivate", { -FLT_MIN, 0 }))
                SetMessage(MT_QuestDeactivate);
        }
...
void FableMenu::MSG_ActivateQuest()
{
    if (!CQuestManager::Get())
        return;
...
void FableMenu::MSG_DeactivateQuest()
{
    if (!CQuestManager::Get())
        return;
...
void FableMenu::ProcessMessages()
{
    switch (m_currentMessage)
    {
    case MT_None:
        break;
    case MT_QuestActivate:
        MSG_ActivateQuest();
        break;
    case MT_QuestDeactivate:
        MSG_DeactivateQuest();
        break;
    default:
        break;
    }
    m_currentMessage = MT_None;
...
{
    ((void(__thiscall*)(int))_addr(0x01CD4940))(ptr);
    TheMenu->ProcessMessages();
}
